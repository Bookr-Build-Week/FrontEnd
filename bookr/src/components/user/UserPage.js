...
function UserPage() {
  ...
  return (
    ...
    {error ? <H1>There are no books on your shelf</H1> : userFavorites.map((i , index) => {
      return (
      <div key={`outerdiv${index}`} style={{width: '100%', border: '1px solid rgba(191, 175, 134, 0.3)',  margin: '0 30px 10px 0px', paddingLeft: '20%'}}>
      <div key={`div${index}`} style={{display: 'flex', width: '100%', marginRight: '30px'}}> 
          ...
          <div>
          <Button class="ui red basic button" key={`delete${index}`} onClick={() => togglePopUp(i.id)} name='delete' size='huge' 
          style={{ color: '#ff0000', cursor: 'pointer', alignSelf: 'flex-start', marginBottom: '5px'}}>Delete</Button>
          {popUp ? 
